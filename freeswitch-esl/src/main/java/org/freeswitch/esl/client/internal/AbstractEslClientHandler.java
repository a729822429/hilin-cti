...
public abstract class AbstractEslClientHandler extends SimpleChannelUpstreamHandler {
     ...
     * fs发送命令线程
     */
    private static final ExecutorService MSG_SEND_THREAD_POOL =
    ...
    public String sendAsyncCommand(Channel channel, final String command) {
        ...
        if (response != null && response.hasHeader(Name.JOB_UUID)) {
...
//        log.info("Received message: {}", EntityUtils.toJsonString(message));
        String contentType = message.getContentType();
